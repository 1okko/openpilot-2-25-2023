...
void AnnotatedCameraWidget::drawHud(QPainter &p) {
  ...
  QRect bar_rect2(rect().left(), rect().bottom() - 120, rect().width(), 60);
  if (devUiEnabled && !mapVisible) {
    p.setPen(Qt::NoPen);
    p.setBrush(QColor(0, 0, 0, 100));
    p.drawRect(bar_rect1);
    drawNewDevUi1(p, bar_rect1.left(), bar_rect1.center().y());
    if (devUiRow == 1) {
      p.setPen(Qt::NoPen);
      p.drawRect(bar_rect2);
      drawNewDevUi2(p, bar_rect2.left(), bar_rect2.center().y());
    }
  }*/

  // ####### 1 ROW #######
  QRect bar_rect1(rect().left(), rect().bottom() - 60, rect().width(), 61);
  if (devUiEnabled && !mapVisible && devUiRow == 1) {
    p.setPen(Qt::NoPen);
    p.setBrush(QColor(0, 0, 0, 100));
    p.drawRect(bar_rect1);
  ...
  if (devUiEnabled) {
    if (devUiRow == 0 || (devUiRow == 1 && mapVisible)) {
  ...
  //rn_btn = devUiEnabled && !mapVisible ? (devUiRow == 1 ? 90 : 60) : 0;
  rn_btn = devUiEnabled && !mapVisible && devUiRow == 1 ? 30 : 0;
...
void AnnotatedCameraWidget::drawRightDevUi2(QPainter &p, int x, int y) {
  ...
  if (torquedUseParams) {
    ...
    snprintf(val_str, sizeof(val_str), "%.2f", latAccelFactorFiltered);

    rh += drawDevUiElementLeft(p, x, ry, val_str, "LAT ACCEL", "m/s²", valueColor);
    ry = y + rh;
  }
  ...
  if (torquedUseParams) {
    ...
    if (liveValid) valueColor = QColor(0, 255, 0, 255);
    snprintf(val_str, sizeof(val_str), "%.2f", frictionCoefficientFiltered);

    rh += drawDevUiElementLeft(p, x, ry, val_str, "FRICTION", "", valueColor);
    ry = y + rh;
  }
...
void AnnotatedCameraWidget::drawRightDevUiBorder(QPainter &p, int x, int y) {
  ...
  p.setPen(QPen(whiteColor(75), 6));
  p.setBrush(QColor(0, 0, 0, 0));
  /*if (!(devUiRow == 0 || (devUiRow == 1 && mapVisible)) && (devUiRow == 1)) {
    rw *= 2;
  }*/
  QRect ldu(x, y, rw, rh);
  p.drawRoundedRect(ldu, 32, 32);
}
...
void AnnotatedCameraWidget::drawNewDevUi1(QPainter &p, int x, int y) {
  int rw = 50;
  int rl = x + 90;
  ...
  if (true) {
    ...
    rw += drawNewDevUiElement(p, rl, y, val_str, "R.D.", units_str, valueColor);
    rl = x + rw;
  }
  ...
  if (true) {
    ...
    rw += drawNewDevUiElement(p, rl, y, val_str, "R.S.", speedUnit.toStdString().c_str(), valueColor);
    rl = x + rw;
  }
  ...
  if (true) {
    ...
    snprintf(val_str, sizeof(val_str), "%.2f%s%s", angleSteers , "°", "");

    rw += drawNewDevUiElement(p, rl, y, val_str, "A.S.", "", valueColor);
    rl = x + rw;
  }
  ...
  if (lateralState == "torque") {
    ...
    if (madsEnabled && latActive) {
      snprintf(val_str, sizeof(val_str), "%.4f", actualLateralAccel);
    } else {
      snprintf(val_str, sizeof(val_str), "-");
    }
    ...
    if (madsEnabled && latActive) {
      ...
      snprintf(val_str, sizeof(val_str), "%.2f%s%s", steerAngleDesired, "°", "");
    } else {
      snprintf(val_str, sizeof(val_str), "-");
    }

    rw += drawNewDevUiElement(p, rl, y, val_str, "D.S.", "", valueColor);
    rl = x + rw;
  }
  ...
  if (true) {
    ...
    rw += drawNewDevUiElement(p, rl, y, val_str, "MEM.", "", valueColor);
    rl = x + rw;
  }
}
...
void AnnotatedCameraWidget::drawNewDevUi2(QPainter &p, int x, int y) {
  ...
  if (true) {
    ...
    rw += drawNewDevUiElement(p, rl, y, val_str, "ACC.", "m/s²", valueColor);
    rl = x + rw;
  }
  ...
  if (true) {
    ...
    rw += drawNewDevUiElement(p, rl, y, val_str, "L.S.", speedUnit.toStdString().c_str(), valueColor);
    rl = x + rw;
  }
  ...
  if (torquedUseParams) {
    ...
    if (liveValid) valueColor = QColor(0, 255, 0, 255);
    snprintf(val_str, sizeof(val_str), "%.4f", frictionCoefficientFiltered);

    rw += drawNewDevUiElement(p, rl, y, val_str, "FRI.", "", valueColor);
    rl = x + rw;
  }
  ...
  // Unit: Newton Meters
    ...
    rw += drawNewDevUiElement(p, rl, y, val_str, "E.T.", "N·dm", valueColor);
    rl = x + rw;
  }
  ...
  if (torquedUseParams) {
    ...
    if (liveValid) valueColor = QColor(0, 255, 0, 255);
    snprintf(val_str, sizeof(val_str), "%.4f", latAccelFactorFiltered);

    rw += drawNewDevUiElement(p, rl, y, val_str, "L.A.", "m/s²", valueColor);
    rl = x + rw;
  }
  ...
  // Unit: Meters
    ...
    rw += drawNewDevUiElement(p, rl, y, "B.D.", data_string, "", valueColor);
    rl = x + rw;
  }
  ...
  if (true) {
    ...
    rw += drawNewDevUiElement(p, rl, y, val_str, "ALT.", "m", valueColor);
    rl = x + rw;
  }
}
